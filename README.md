...
一个面向中文口播与教程类知识视频的 Codex 插件。它把脚本、配音、视觉方案、分镜、素材生产、时间线装配、结构校验和审阅打包拆成可恢复、可审批、可追踪的工作流。
...
关键创意决策会持久化为审批产物。缺少审批、输入过期、合同不匹配或项目状态无法重放时，协调器会停止，而不是猜测后继续。
...
需要 Python 3.9+ 和支持个人插件目录的 Codex Desktop。

克隆仓库：

```bash
git clone git@github.com:Fantasy421/video-production-toolkit.git
...
注册到个人插件目录。开发时推荐软链接模式，仓库更新后不需要重复复制：
...
插件会根据项目阶段选择下一项合法任务。它不会绕过视觉方向、配音来源、分镜或代表性片段等审批门。
