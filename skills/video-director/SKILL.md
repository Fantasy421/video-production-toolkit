...
2. Choose exactly one ready task: one action slice, never a fan-out. Do not generate media from this routing skill; it never synthesizes, imports, or analyzes audio.
...
4. Load only that child entrypoint and hand it only artifact IDs, paths, summaries, and contract results. The child receives its claimed task envelope and returns one task-result envelope; persist the result through the task state manager.
